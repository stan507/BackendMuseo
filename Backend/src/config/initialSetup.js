...
export async function seedDatabase() {
    try {
        ...
        if (respondeCount === 0 && quizzCountActual > 0 && admin) {
            ...
            const quizHelice = await quizzRepo.findOne({ where: { id_exhibicion: "helice" } });
            if (quizHelice) {
                const preguntasHeliceQuery = `
                    SELECT 
                        p.id_pregunta,
                        p.titulo,
                        r.id_respuesta,
                ...
                const preguntasHeliceRaw = await AppDataSource.query(preguntasHeliceQuery, [quizHelice.id_quizz]);


                const preguntasHeliceMap = {};
                preguntasHeliceRaw.forEach(row => {
                    if (!preguntasHeliceMap[row.id_pregunta]) {
                        preguntasHeliceMap[row.id_pregunta] = {
                            id_pregunta: row.id_pregunta,
                            titulo: row.titulo,
                            respuestas: []
                        };
                    ...
                    if (row.id_respuesta) {
                        preguntasHeliceMap[row.id_pregunta].respuestas.push({
                            id_respuesta: row.id_respuesta,
                            texto: row.respuesta_texto,
                            es_correcta: row.es_correcta
                        });
                ...
                const preguntasHelice = Object.values(preguntasHeliceMap);

                if (preguntasHelice.length >= 2) {

                    const primeraRespuestaCorrecta = preguntasHelice[0].respuestas.find(r => r.es_correcta);

                    const abandonoHelice = {
                        id_usuario: admin.id_usuario,
                        id_quizz: quizHelice.id_quizz,
                        correctas: 1,
                        tiempo_segundos: 15,
                        respuestas_detalle: [
                            {
                                id_pregunta: preguntasHelice[0].id_pregunta,
                                id_respuesta_seleccionada: primeraRespuestaCorrecta.id_respuesta,
                                es_correcta: true,
                                texto_pregunta: preguntasHelice[0].titulo,
                            }
                        ]
                    };

                    respuestasEjemplo.push(abandonoHelice);
                }
            }

            if (respuestasEjemplo.length > 0) {
                await respondeRepo.save(respuestasEjemplo);
                console.log(`  Se insertaron ${respuestasEjemplo.length} respuestas de ejemplo con detalles completos (incluyendo 1 abandono).`);
            }
        } else {
